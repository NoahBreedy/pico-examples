...
static void ssd1351_flush_dma(uint8_t *ptr, size_t len)
{
    ...
    gpio_put(SPI_DC, WRITE_DATA);
    spi_write_blocking(SPI_NUM, ptr, len);
...
void ssd1351_clear()
{
    ...
    memset(buffer, 0x00, sizeof(oled_dma_1));
}
...
inline void ssd1351_draw_pixel_spi(uint16_t col,point_t pos){
        uint8_t buffer[3];
        ssd1351_write(SSD_CMD_SETCOLUMN,buffer,2);
}
...
{   
    // y offset 
    uint8_t *buffer = get_buffer(type);
    memcpy(buffer + ((pos.y+OLED_Y_OFFEST)*WIDTH*2) + (pos.x*2), &colour, 2);
   // memcpy(buffer + ((pos.y+OLED_Y_OFFEST)*WIDTH*2) + (pos.x*2), &colour, 2);
    //memcpy(buffer + ((((pos.y*2+OLED_Y_OFFEST)*WIDTH) + (pos.x))*2), &colour, 2);
}
